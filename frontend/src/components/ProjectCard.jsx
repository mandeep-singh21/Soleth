...
export default function ProjectCard({ project, onSelect }) {
  ...
  return (
    <div
      ...
      className="gpay-card p-6 cursor-pointer group relative flex flex-col justify-between"
    >
      {/* Top Meta */}
      <div>
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-[#11141c] border border-white/10 text-[11px] font-mono font-semibold text-slate-400">
              #{project.id}
            </span>
            <span
          ...
          <div className="flex items-center gap-1.5">
            {isClient && (
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#4285F4]/20 text-[#8ab4f8] border border-[#4285F4]/40">
                You (Client)
              </span>
            )}
            {isFreelancer && (
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#34A853]/20 text-[#81c995] border border-[#34A853]/40">
                You (Freelancer)
              </span>
            )}
          </div>
        ...
        {/* Title */}
        <h3 className="text-lg font-bold text-white group-hover:text-[#4285F4] transition-colors mb-2 line-clamp-1">
          {project.name}
        </h3>
        ...
        <div className="space-y-1.5 text-xs text-slate-400 mb-4 font-mono">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 font-sans font-medium">Client:</span>
            <span className="text-slate-300">{shortenAddress(project.client)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-400 font-sans font-medium">Freelancer:</span>
        ...
        <div className="my-4 pt-3 border-t border-white/5">
          <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
            <span className="font-medium">Milestones ({paidCount}/{totalMilestones} Paid)</span>
            <span className="font-mono text-[#81c995] font-bold">{Math.round(progressPercent)}%</span>
          </div>

          {/* Progress Bar */}
          <div className="w-full h-2 rounded-full bg-[#11141c] overflow-hidden flex mb-3 border border-white/5">
            <div
              style={{ width: `${progressPercent}%` }}
              className="h-full bg-gradient-to-r from-[#4285F4] to-[#34A853] transition-all duration-500 rounded-full"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {project.milestones.map((m, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`px-2.5 py-0.5 rounded-full border text-[10px] font-mono flex items-center gap-1 ${msConfig.color}`}
      ...
      <div className="pt-4 border-t border-white/5 flex items-center justify-between mt-2">
        ...
        <div className="flex items-center gap-1 text-xs font-semibold text-[#4285F4] group-hover:translate-x-1 transition-transform">
          <span>Manage</span>
